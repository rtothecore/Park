...
public class BluetoothPrintService {
	...
	public synchronized void connected(BluetoothSocket socket, BluetoothDevice device, final String socketType) {
		...
		Message msg = mHandler.obtainMessage(NTSApp.MESSAGE_DEVICE_NAME);
		Bundle bundle = new Bundle();
		bundle.putString(NTSApp.DEVICE_NAME, device.getName());
		msg.setData(bundle);
	...
	private void connectionFailed() {
		...
		Message msg = mHandler.obtainMessage(NTSApp.MESSAGE_TOAST);
		Bundle bundle = new Bundle();
		bundle.putString(NTSApp.TOAST, "Unable to connect device");
		msg.setData(bundle);
		mHandler.sendMessage(msg);
	...
	private void connectionLost() {
		...
		Message msg = mHandler.obtainMessage(NTSApp.MESSAGE_TOAST);
		Bundle bundle = new Bundle();
		bundle.putString(NTSApp.TOAST, "Device connection was lost");
		msg.setData(bundle);
		mHandler.sendMessage(msg);
		...
		public void run() {
			...
			while(true) {
				try {
					...
					// buffer can be over-written by next input stream data, so
					// it should be copied
					byte[] rcvData = new byte[bytes];
					rcvData = Arrays.copyOf(buffer, bytes);

					// Send the obtained bytes to the UI Activity
					mHandler.obtainMessage(NTSApp.MESSAGE_READ, bytes, -1, rcvData).sendToTarget();
				}
				catch(IOException e) {
